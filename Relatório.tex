...
\titulo{Implementação em C++: Single Image Haze Removal Using Dark Channel Prior}
...
\instituicao{%
	...
	\par
FAMB - FÍSICA APLICADA À MEDICINA E BIOLOGIA%\includegraphics[width=10cm]{texfiles/ffclrp.pcx}
}

\preambulo{Trabalho Final}
...
\bottomrule
\end{tabularx}
\legend{Fonte: \cite{guided_filtering}}
\end{table}
\clearpage
...
\begin{figure}[h]
	...
	\begin{center}
		\caption{\label{diagrama_bloco}Diagrama de Blocos}
		%\begin{tikzpicture}[auto, node distance=1cm,>=latex']
\tikzstyle{block} = [draw, fill=blue!20, rectangle, 
minimum height=3em,  font=\footnotesize]
...
\tikzstyle{pinstyle} = [pin edge={to-,thin,black}]
		...
		\node [block, minimum height=2cm] (image) {Imagem Haze};
		\node [block, right=of image] (minima) {minima()};
		\node [block, right=of minima] (dark) {dark\_channel()};
		\node [block, right=of dark] (achaA) {achaA()};
		\node [block, below=of image] (corrigeA) {corrigeA()};
		\node [block, right=of corrigeA] (minima2) {minima()};
		\node [block, right=of minima2] (dark2) {dark\_channel()};
		\node [block, below=of dark2] (calculaT) {calculaT()};
		\node [block, left=of calculaT] (matting) {matting2()};
		\node [block, right=of calculaT] (guided) {guided\_filtering()};
		\node [block, below=of calculaT] (tirahaze) {tiraHaze()};
		\node [block, below=of tirahaze] (out) {Imagem Saída};
		
		\draw[->] (image.east) -- (minima.west);
		\draw[->] (minima.east) -- (dark.west);
		\draw[->] (dark.east) -- (achaA.west);
		\draw[->] (image.south) -- (corrigeA.north);
		\draw[->] (corrigeA.east) -- (minima2.west);
		\draw[->] (minima2.east) -- (dark2.west);
		\draw[->] (dark2.south) -- (calculaT.north);
		\draw[->] (achaA.south west) -- (corrigeA.north east);
		\draw[->] (calculaT.west) -- (matting.east);
		\draw[->] (calculaT.east) -- (guided.west);
		\draw[->] (matting.south east) -- (tirahaze.north west);
		\draw[->] (guided.south west) -- (tirahaze.north east);
		\draw[->] (tirahaze.south) -- (out.north);
